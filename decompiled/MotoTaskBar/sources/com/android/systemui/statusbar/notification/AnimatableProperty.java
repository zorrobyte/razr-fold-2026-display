package com.android.systemui.statusbar.notification;

import android.util.FloatProperty;
import android.util.Property;
import android.view.View;
import com.android.systemui.res.R$id;

/* JADX INFO: loaded from: classes.dex */
public abstract class AnimatableProperty {
    public static final AnimatableProperty ABSOLUTE_X;
    public static final AnimatableProperty ABSOLUTE_Y;
    public static final AnimatableProperty ALPHA;
    public static final AnimatableProperty HEIGHT;
    public static final AnimatableProperty SCALE_X;
    public static final AnimatableProperty SCALE_Y;
    public static final AnimatableProperty TRANSLATION_X;
    public static final AnimatableProperty WIDTH;
    public static final AnimatableProperty X;
    public static final AnimatableProperty Y;

    static {
        Property property = View.X;
        int i = R$id.x_animator_tag;
        int i2 = R$id.x_animator_tag_start_value;
        int i3 = R$id.x_animator_tag_end_value;
        X = from(property, i, i2, i3);
        Y = from(View.Y, R$id.y_animator_tag, R$id.y_animator_tag_start_value, R$id.y_animator_tag_end_value);
        TRANSLATION_X = from(View.TRANSLATION_X, i, i2, i3);
        SCALE_X = from(View.SCALE_X, R$id.scale_x_animator_tag, R$id.scale_x_animator_start_value_tag, R$id.scale_x_animator_end_value_tag);
        SCALE_Y = from(View.SCALE_Y, R$id.scale_y_animator_tag, R$id.scale_y_animator_start_value_tag, R$id.scale_y_animator_end_value_tag);
        ALPHA = from(View.ALPHA, R$id.alpha_animator_tag, R$id.alpha_animator_start_value_tag, R$id.alpha_animator_end_value_tag);
        ABSOLUTE_X = from(new FloatProperty("ViewAbsoluteX") { // from class: com.android.systemui.statusbar.notification.AnimatableProperty.1
            @Override // android.util.Property
            public Float get(View view) {
                Object tag = view.getTag(R$id.absolute_x_current_value);
                return tag instanceof Float ? (Float) tag : (Float) View.X.get(view);
            }

            @Override // android.util.FloatProperty
            public void setValue(View view, float f) {
                view.setTag(R$id.absolute_x_current_value, Float.valueOf(f));
                View.X.set(view, Float.valueOf(f));
            }
        }, R$id.absolute_x_animator_tag, R$id.absolute_x_animator_start_tag, R$id.absolute_x_animator_end_tag);
        ABSOLUTE_Y = from(new FloatProperty("ViewAbsoluteY") { // from class: com.android.systemui.statusbar.notification.AnimatableProperty.2
            @Override // android.util.Property
            public Float get(View view) {
                Object tag = view.getTag(R$id.absolute_y_current_value);
                return tag instanceof Float ? (Float) tag : (Float) View.Y.get(view);
            }

            @Override // android.util.FloatProperty
            public void setValue(View view, float f) {
                view.setTag(R$id.absolute_y_current_value, Float.valueOf(f));
                View.Y.set(view, Float.valueOf(f));
            }
        }, R$id.absolute_y_animator_tag, R$id.absolute_y_animator_start_tag, R$id.absolute_y_animator_end_tag);
        WIDTH = from(new FloatProperty("ViewWidth") { // from class: com.android.systemui.statusbar.notification.AnimatableProperty.3
            @Override // android.util.Property
            public Float get(View view) {
                Object tag = view.getTag(R$id.view_width_current_value);
                return tag instanceof Float ? (Float) tag : Float.valueOf(view.getWidth());
            }

            @Override // android.util.FloatProperty
            public void setValue(View view, float f) {
                view.setTag(R$id.view_width_current_value, Float.valueOf(f));
                view.setRight((int) (view.getLeft() + f));
            }
        }, R$id.view_width_animator_tag, R$id.view_width_animator_start_tag, R$id.view_width_animator_end_tag);
        HEIGHT = from(new FloatProperty("ViewHeight") { // from class: com.android.systemui.statusbar.notification.AnimatableProperty.4
            @Override // android.util.Property
            public Float get(View view) {
                Object tag = view.getTag(R$id.view_height_current_value);
                return tag instanceof Float ? (Float) tag : Float.valueOf(view.getHeight());
            }

            @Override // android.util.FloatProperty
            public void setValue(View view, float f) {
                view.setTag(R$id.view_height_current_value, Float.valueOf(f));
                view.setBottom((int) (view.getTop() + f));
            }
        }, R$id.view_height_animator_tag, R$id.view_height_animator_start_tag, R$id.view_height_animator_end_tag);
    }

    public static AnimatableProperty from(final Property property, final int i, final int i2, final int i3) {
        return new AnimatableProperty() { // from class: com.android.systemui.statusbar.notification.AnimatableProperty.7
            @Override // com.android.systemui.statusbar.notification.AnimatableProperty
            public int getAnimationEndTag() {
                return i3;
            }

            @Override // com.android.systemui.statusbar.notification.AnimatableProperty
            public int getAnimationStartTag() {
                return i2;
            }

            @Override // com.android.systemui.statusbar.notification.AnimatableProperty
            public int getAnimatorTag() {
                return i;
            }

            @Override // com.android.systemui.statusbar.notification.AnimatableProperty
            public Property getProperty() {
                return property;
            }
        };
    }

    public abstract int getAnimationEndTag();

    public abstract int getAnimationStartTag();

    public abstract int getAnimatorTag();

    public abstract Property getProperty();
}
