...
public final class RoundedPolygon {
    ...
    public final float[] calculateMaxBounds(float[] fArr) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            long jM1067pointOnCurveOOQOV4g$graphics_shapes = cubic.m1067pointOnCurveOOQOV4g$graphics_shapes(0.5f);
            fMax = Math.max(fMax, Math.max(fDistanceSquared, Utils.distanceSquared(PointKt.m1076getXDnnuFBc(jM1067pointOnCurveOOQOV4g$graphics_shapes) - getCenterX(), PointKt.m1077getYDnnuFBc(jM1067pointOnCurveOOQOV4g$graphics_shapes) - getCenterY())));
        }
    ...
    public final float getCenterX() {
        return PointKt.m1076getXDnnuFBc(this.center);
    }

    public final float getCenterY() {
        return PointKt.m1077getYDnnuFBc(this.center);
    }
    ...
    public final RoundedPolygon transformed(PointTransformer pointTransformer) {
        ...
        long jM1082transformedso9K2fw = PointKt.m1082transformedso9K2fw(this.center, pointTransformer);
        List listCreateListBuilder = CollectionsKt.createListBuilder();
        int size = this.features.size();
        for (int i = 0; i < size; i++) {
            listCreateListBuilder.add(((Feature) this.features.get(i)).transformed(pointTransformer));
        }
        return new RoundedPolygon(CollectionsKt.build(listCreateListBuilder), jM1082transformedso9K2fw, null);
    }
}
