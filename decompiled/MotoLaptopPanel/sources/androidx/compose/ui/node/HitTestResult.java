...
public final class HitTestResult implements List, KMappedMarker {
    ...
    private final long m1314findBestHitDistancefn2tFes() {
        ...
        if (i <= lastIndex) {
            while (true) {
                long jM1310constructorimpl = DistanceAndFlags.m1310constructorimpl(this.distanceFromEdgeAndFlags.get(i));
                if (DistanceAndFlags.m1309compareTo9YPOF3E(jM1310constructorimpl, jDistanceAndFlags$default) < 0) {
                    jDistanceAndFlags$default = jM1310constructorimpl;
                }
                if ((DistanceAndFlags.m1311getDistanceimpl(jDistanceAndFlags$default) < 0.0f && DistanceAndFlags.m1313isInLayerimpl(jDistanceAndFlags$default)) || i == lastIndex) {
                    break;
                }
                i++;
            }
    ...
    public final boolean hasHit() {
        long jM1314findBestHitDistancefn2tFes = m1314findBestHitDistancefn2tFes();
        return DistanceAndFlags.m1311getDistanceimpl(jM1314findBestHitDistancefn2tFes) < 0.0f && DistanceAndFlags.m1313isInLayerimpl(jM1314findBestHitDistancefn2tFes) && !DistanceAndFlags.m1312isInExpandedBoundsimpl(jM1314findBestHitDistancefn2tFes);
    }
    ...
    public final void hitExpandedTouchBounds(Modifier.Node node, boolean z, Function0 function0) {
        ...
        if (!DistanceAndFlags.m1312isInExpandedBoundsimpl(jM1314findBestHitDistancefn2tFes)) {
            if (DistanceAndFlags.m1311getDistanceimpl(jM1314findBestHitDistancefn2tFes) > 0.0f) {
        ...
        function0.invoke();
        this.hitDepth = i4;
        if (DistanceAndFlags.m1311getDistanceimpl(m1314findBestHitDistancefn2tFes()) < 0.0f) {
    ...
    public final boolean isHitInMinimumTouchTargetBetter(float f, boolean z) {
        ...
        return DistanceAndFlags.m1309compareTo9YPOF3E(m1314findBestHitDistancefn2tFes(), HitTestResultKt.DistanceAndFlags$default(f, z, false, 4, null)) > 0;
    }
    ...
    public final void speculativeHit(Modifier.Node node, float f, boolean z, Function0 function0) {
        if (this.hitDepth == CollectionsKt.getLastIndex(this)) {
            ...
            function0.invoke();
            this.hitDepth = i;
            if (this.hitDepth + 1 == CollectionsKt.getLastIndex(this) || DistanceAndFlags.m1312isInExpandedBoundsimpl(m1314findBestHitDistancefn2tFes())) {
        ...
        function0.invoke();
        this.hitDepth = i3;
        long jM1314findBestHitDistancefn2tFes2 = m1314findBestHitDistancefn2tFes();
        if (this.hitDepth + 1 >= CollectionsKt.getLastIndex(this) || DistanceAndFlags.m1309compareTo9YPOF3E(jM1314findBestHitDistancefn2tFes, jM1314findBestHitDistancefn2tFes2) <= 0) {
            removeNodesInRange(this.hitDepth + 1, size());
        } else {
            removeNodesInRange(i2 + 1, DistanceAndFlags.m1312isInExpandedBoundsimpl(jM1314findBestHitDistancefn2tFes2) ? this.hitDepth + 2 : this.hitDepth + 1);
        }
        this.hitDepth = i2;
    }
