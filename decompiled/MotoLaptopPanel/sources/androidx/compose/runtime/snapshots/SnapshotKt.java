    ...
    */
    ...
    public static final StateRecord overwritableRecord(StateRecord stateRecord, StateObject stateObject, Snapshot snapshot, StateRecord stateRecord2) {
        ...
        if (snapshot.getReadOnly()) {
            snapshot.mo654recordModified$runtime_release(stateObject);
        }
    ...
    public static final StateRecord writableRecord(StateRecord stateRecord, StateObject stateObject, Snapshot snapshot) {
        ...
        long snapshotId = snapshot.getSnapshotId();
        StateRecord stateRecord2 = readable(stateRecord, snapshotId, snapshot.getInvalid$runtime_release());
        if (stateRecord2 == null) {
            readError();
            throw new KotlinNothingValueException();
        }
        if (stateRecord2.getSnapshotId$runtime_release() == snapshot.getSnapshotId()) {
            return stateRecord2;
        }
        synchronized (getLock()) {
            stateRecordNewWritableRecordLocked = readable(stateObject.getFirstStateRecord(), snapshotId, snapshot.getInvalid$runtime_release());
            if (stateRecordNewWritableRecordLocked == null) {
                readError();
                throw new KotlinNothingValueException();
            }
            if (stateRecordNewWritableRecordLocked.getSnapshotId$runtime_release() != snapshotId) {
                stateRecordNewWritableRecordLocked = newWritableRecordLocked(stateRecordNewWritableRecordLocked, stateObject, snapshot);
            }
        }
        stateRecordNewWritableRecordLocked.getClass();
        if (stateRecord2.getSnapshotId$runtime_release() != SnapshotId_jvmKt.toSnapshotId(1)) {
            snapshot.mo654recordModified$runtime_release(stateObject);
        }
        return stateRecordNewWritableRecordLocked;
