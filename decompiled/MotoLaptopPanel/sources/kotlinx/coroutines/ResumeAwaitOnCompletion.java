...
final class ResumeAwaitOnCompletion extends JobNode {
    ...
    public void invoke(Throwable th) {
        ...
        if (state$external__kotlinx_coroutines__linux_glibc_common__kotlinx_coroutines_host instanceof CompletedExceptionally) {
            ...
            cancellableContinuationImpl.resumeWith(Result.m2192constructorimpl(ResultKt.createFailure(((CompletedExceptionally) state$external__kotlinx_coroutines__linux_glibc_common__kotlinx_coroutines_host).cause)));
        } else {
            CancellableContinuationImpl cancellableContinuationImpl2 = this.continuation;
            Result.Companion companion2 = Result.Companion;
            cancellableContinuationImpl2.resumeWith(Result.m2192constructorimpl(JobSupportKt.unboxState(state$external__kotlinx_coroutines__linux_glibc_common__kotlinx_coroutines_host)));
        }
    }
}
